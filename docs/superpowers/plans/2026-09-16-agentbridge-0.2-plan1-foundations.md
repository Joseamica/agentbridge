...
- Produces:
  ...
  - Paging rule, per window, with `trusted = max(NOSTR.minTrustedRelayLimit, largest page this relay returned during this call)`:
    ...
    2. A page with at least `min(limit, trusted)` events may be truncated. Move `until` to the second of its k-th newest distinct valid event, k = min(limit, trusted) − entries that are not distinct valid events (k ≤ 0 → incomplete), if that second is older than `until` (inclusive; the overlap is de-duplicated), and continue. If the k-th newest event sits at the `until` second, escalate the limit 200 → 400 → 800; past 800 the window is incomplete.
  ...
  - Accepted limitations (Global Constraints): a relay that caps filter limits below 100 can hide same-second ties; a relay that caps below the requested limit or below 100 while delivering extra events before EOSE, whose truncation varies between queries, or that truncates other than newest-first can hide events. Rulings 16–18 and 20 of plan 1 execution: k-th newest cut, page size clamped to the limit, 250 queries per window, an unreadable page completes a window only after a larger page from the same relay.
...
- `reservePublish` is not idempotent and `beforeSend` runs once per relay write (up to 5 relays × one auth retry, in parallel): plan 2 must memoize the reservation per claim attempt, and `over_budget` means `postpone`, not `markFailed`.
- `BoardConnection` detects half-open sockets with a ping heartbeat; `BoardPool.close()` stops connects, queries, publishes and live loops; `recoverHistory` takes an abort signal.
- Nested `Store.tx` joins the outer transaction without savepoints: code that catches an inner failure still commits the outer transaction's partial work.
- The live REQ has no `limit`, so relay default caps truncate the backlog on reconnect: plan 2 re-runs history on reconnect and periodically, and must account for a wrap that is still waiting in the live queue (already in `SeenIds`) when history marks its window complete.
- `authorize` needs the generation a message was created for; `OutboxItem` and the receipt/answer/rejected rumors do not carry it, so plan 2 must derive it inside `claimDue`'s transaction.
