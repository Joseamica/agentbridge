...
- Produces:
  ...
  - `regenerateRequestDecision(store, { identity, senderPubkey, requestId, replyRelays, now }): 'enqueued' | 'too_soon' | 'nothing' | 'abandoned'`: a decision already sent is resent at most once every `NOSTR.regenerationIntervalSeconds`, counted from `requests.decision_resent_at` (or `decided_at`), so the limit survives revocation deleting its outbox rows. A decision never sent yet goes out at once. The resend clock (`decision_resent_at`) only advances when the outbox actually (re)armed the row (`enqueued`/`postponed_cap`/`regenerated`); an outbox row still pending from an earlier send (`already_pending`) still reports `'enqueued'` but leaves the clock alone, and a row the outbox abandoned reports `'abandoned'` instead of a false `'enqueued'`.
